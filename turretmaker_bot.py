  ...
  def tryBuildGunner(self, ct: Controller):
    ...
    for dir in pfind.OCT_DIRECTIONS:
      ...
      if not self.posIsInbounds(ct, testPos):
        continue
      if ct.get_tile_env(testPos) != Environment.EMPTY or not ct.can_build_gunner(testPos, testPos.direction_to(aimingPos)):
        continue
